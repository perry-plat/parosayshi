Create one horizontal animation strip for Codex pet `pico`, state `running-right`.
...
Identity: same pet in every frame: A small friendly flat-vector studio buddy for a product designer who is learning to build: rounded teal body, warm cream face panel, expressive navy eyes, tiny coral pencil tucked behind one ear, compact whole-body silhouette, curious and quietly determined.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `flat-vector`: Flat vector-style mascot with simple geometric forms, crisp color areas, clean outline, and minimal shading. User style notes: Warm editorial product-design energy; simple bold shapes, crisp dark outline, no text, no logos, no scenery..
